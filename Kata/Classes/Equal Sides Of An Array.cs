    ...
    public static class Equal_Sides_Of_An_Array
    {
        public static int FindEvenIndex(int[] arr)
        {
            var filter = arr
                ...
                .Where((_, i) => arr.Take(i).Sum() == arr.Skip(i + 1).Sum());
        ...
        public static int FindEvenIndexCls(int[] arr)
        {
            if (arr.Length > 0 && arr.Length < 1000)
            {
                ...
                {
                    if (i == 0) sumLeft = 0;
                    else sumLeft += arr[i - 1];
                    int sumRight = 0;
                    for (int j = arr.Length - 1; j >= i; j--)
                    {
                        if (j == arr.Length - 1) sumRight = 0;
                        else sumRight += arr[j + 1];
                        if (j == i && sumLeft.Equals(sumRight))
                            return i;
                    }
                }
            }
